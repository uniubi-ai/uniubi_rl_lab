...
from uniubi_rl_lab.assets.robots.uniubi import UNIUBI_CYVET_CFG as CYVET_ROBOT_CFG
from uniubi_rl_lab.tasks.locomotion import mdp

BASE_BODY_NAME = "BASE_LINK"
FOOT_BODY_NAMES = ".*FOOT_LINK"
ACTIVE_JOINT_NAMES = [".*_ABAD_JOINT", ".*_HIP_JOINT", ".*_KNEE_JOINT"]
UNDESIRED_BODY_NAMES = ["BASE_LINK", ".*ABAD_LINK", ".*HIP_LINK", ".*KNEE_LINK"]
...
@configclass
class CyvetSceneCfg(InteractiveSceneCfg):
    ...
    robot: ArticulationCfg = CYVET_ROBOT_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class RewardsCfg:
    ...
    joint_pos = RewTerm(
        ...
        weight=-0.7,
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=ACTIVE_JOINT_NAMES),
...
class TerminationsCfg:
    ...
    base_contact = DoneTerm(
        ...
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=BASE_BODY_NAME), "threshold": 1.0},
    )
...
@configclass
class CyvetEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the locomotion velocity-tracking environment."""

    scene: CyvetSceneCfg = CyvetSceneCfg(num_envs=4096, env_spacing=2.5)
...
@configclass
class CyvetPlayEnvCfg(CyvetEnvCfg):
    def __post_init__(self):
        super().__post_init__()
